...
void USART_DMA_Config(void)
{
  ...
  // 将USART与DMA联系起来
  // UartHandle，是Uart的句柄结构体
  // DMA_Handle，是DMA的句柄结构体
  // hdmatx 是外设/DMA联系的桥梁
  // hdmatx 是DMA_HandleTypeDef类型结构体指针变量 (DMA_HandleTypeDef  *hdmatx;)
  // PS：DMA_HandleTypeDef类型 在 USART_HandleTypeDef中定义，
  // 通过这个结构体指针 DMA_HandleTypeDef * hdmatx 完成外设/存储器 的DMA连接
  
  // PS： 区分发射连接DMA 与 接收连接DMA
