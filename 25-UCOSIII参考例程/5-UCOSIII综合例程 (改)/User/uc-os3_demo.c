...
{   
    ...
    while(1)
    {
        ...
        cnt = OSSemPend (&KEY1_binary_sem , 0 , OS_OPT_PEND_BLOCKING , 0 , &err);  /* 获取二值信号量 */
        printf("此时KEY1二值信号量为: %d\r\n",cnt);
        ...
        os_flag = OSFlagPost (&flag1 , FLAG_BIT0 , OS_OPT_POST_FLAG_SET , &err);        /* 事件标志位置位 */
        OSSchedLock(&err);           /* 任务调度锁上锁 */ 
        printf("ADC事件标志帧1写完毕\r\n");
        printf("此时事件标志位为0x%02x\r\n", os_flag);
        OSSchedUnlock(&err);         /* 任务调度锁解锁 */   
        
        os_flag = OSFlagPost (&flag2 , FLAG_BIT0 , OS_OPT_POST_FLAG_SET , &err);        /* 事件标志位置位 */
        OSSchedLock(&err);           /* 任务调度锁上锁 */ 
        printf("ADC事件标志帧2写完毕\r\n");
        printf("此时事件标志位为0x%02x\r\n", os_flag);
        OSSchedUnlock(&err);         /* 任务调度锁解锁 */           
        ...
        OSSchedLock(&err);           /* 任务调度锁上锁 */ 
        printf("ADC任务，执行完成ADC task，记时信号量为%d\r\n", cnt);
        OSSchedUnlock(&err);         /* 任务调度锁解锁 */   
...
void I2C_task(void *p_arg)
{
    ...
    {
        /* 读信号：  信号量句柄结构体， 最大等待时间(0 表示一直等) ，阻塞任务； 返回值：信号量计数值 */
        cnt = OSSemPend (&KEY2_binary_sem , 0 , OS_OPT_PEND_BLOCKING , 0 , &err);  /* 获取二值信号量 */
        printf("此时KEY2二值信号量为: %d\r\n",cnt);
        ...
        /* 写事件标志帧：  事件标志组句柄结构体， 事件标志组哪一位写入， 对这一位的操作 */
        os_flag = OSFlagPost (&flag1 , FLAG_BIT1 , OS_OPT_POST_FLAG_SET , &err);        /* 事件标志位置位 */
        OSSchedLock(&err);           /* 任务调度锁上锁 */ 
        printf("I2C事件标志帧写完毕\r\n");
        printf("I2C任务，此时事件标志位为0x%02x\r\n", os_flag);
        OSSchedUnlock(&err);         /* 任务调度锁解锁 */   
        
        os_flag = OSFlagPost (&flag2 , FLAG_BIT1 , OS_OPT_POST_FLAG_SET , &err);        /* 事件标志位置位 */
        OSSchedLock(&err);           /* 任务调度锁上锁 */ 
        printf("I2C事件标志帧2写完毕\r\n");
        printf("I2C任务，此时事件标志位为0x%02x\r\n", os_flag);
        OSSchedUnlock(&err);         /* 任务调度锁解锁 */    
...
void KEY1_task(void *p_arg)
{
    ...
    while(1)
    {
        ...
        if(key == KEY1_PRES)
        {
            ...
            cnt = OSSemPost (&KEY1_binary_sem , OS_OPT_POST_1 , &err);                 /* 释放二值信号量 */
            OSSchedLock(&err);           /* 任务调度锁上锁 */
            printf("KEY1按下，成功写入\r\n");
            printf("此时KEY1二值信号量为: %d\r\n",cnt);
            OSSchedUnlock(&err);         /* 任务调度锁解锁 */       
...
//        {
//            /* 写信号量：  二值信号量句柄， 写入方式(只给一个写入) */
//            cnt = OSSemPost (&KEY2_binary_sem , OS_OPT_POST_1 , &err);                 /* 释放二值信号量 */
//            OSSchedLock(&err);           /* 任务调度锁上锁 */
//            printf("KEY2按下，成功写入\r\n");
//            printf("此时KEY2二值信号量为: %d\r\n",cnt);
//            OSSchedUnlock(&err);         /* 任务调度锁解锁 */       
...
void SHOW1_task(void *p_arg)
{
    ...
    {
        /* 读事件标志组 */
        os_flag = OSFlagPend (&flag1 ,   /* 哪个事件标志组 */
        ...
        if(err!=0)  printf("此时的错误码为: %d\r\n", err);
        printf("SHOW1任务，此时事件标志位为0x%02x\r\n", os_flag);
...
void SHOW2_task(void *p_arg)
{
    ...
    {
        /* 读事件标志组 */
        os_flag = OSFlagPend (&flag2 ,   /* 哪个事件标志组 */
                    FLAG_BIT0 | FLAG_BIT1 ,   /* 事件标志位 */
                    0 ,      /* 任务挂起等待时间 */
                    /* 函数选项： 等待“flags”中的所有指定位被置1，当等待到指定位后,清0对应位，标志组不满足条件时挂起任务 */
        ...
        OSSchedLock(&err);           /* 任务调度锁上锁 */  
        if(err!=0)  printf("SHOW2任务，此时的错误码为: %d\r\n", err);     // 错误码 25008，不阻塞就会一直报错，所以必须设为阻塞模式
        printf("SHOW2任务，此时事件标志位为0x%02x\r\n", os_flag);
        OSSchedUnlock(&err);         /* 任务调度锁解锁 */    
      
        // 如果不置位将会一直进入循环
        os_flag = OSFlagPost (&flag2 , FLAG_BIT1|FLAG_BIT0 , OS_OPT_POST_FLAG_CLR , &err);        /* 事件标志位置位 */
...
void Task_StkChk(void *p_arg) 
{
    ...
    {   
        OSSchedLock(&err);           /* 任务调度锁上锁 */
        OSTaskStkChk (&ADCTask_TCB, &free, &used, &err);   //  把统计任务本身的堆栈使用量也打印出来, 然后从实验结果看看我们设置256字节给它是不是真的合适  
...
读取到的I2C值为: 4.900000
I2C队列写完毕
SHOW1任务，此时事件标志位为0x03
接收到数据，采集到的AD电压为3.273413, 光照强度为4.900000
I2C事件标志帧写完毕
I2C任务，此时事件标志位为0x00
SHOW2任务，此时事件标志位为0x03
SHOW2任务，此时事件标志位为0x00
I2C事件标志帧2写完毕
I2C任务，此时事件标志位为0x00
执行I2C task，记时信号量为110
KEY2按下，成功写入
此时KEY2二值信号量为: 0 (亮红灯)
