...
int main(void) 
{
  ...
  printf("野火F103 停止模式例程\r\n");
	
	printf("\r\n 实验说明：\r\n");

	printf("1.本程序中，绿灯表示STM32正常运行，红灯表示睡眠状态，蓝灯表示刚从停止状态被唤醒\r\n");
	printf("2.程序运行一段时间后自动进入停止状态，在停止状态下，可使用KEY1或KEY2唤醒\r\n");
	printf("3.本实验执行这样一个循环：\r\n ------》亮绿灯(正常运行)->亮红灯(停止模式)->按KEY1或KEY2唤醒->亮蓝灯(刚被唤醒)-----》\r\n");
	printf("4.在停止状态下，DAP下载器无法给STM32下载程序，\r\n 可按KEY1、KEY2唤醒后下载，\r\n 或按复位键使芯片处于复位状态，然后在电脑上点击下载按钮，再释放复位按键，即可下载\r\n");
	...
	{	
		...
		// PWR_MAINREGULATOR_ON表示调节器在主模式，PWR_LOWPOWERREGULATOR_ON表示调节器在低功耗模式，低功耗模式唤醒时间更长
		// PWR_SLEEPENTRY_WFI 表示通过WFI(中断)指令进入睡眠模式，PWR_SLEEPENTRY_WFE 表示通过WFE(事件)指令进入睡眠模式
		// 之后使用任意外部中断可从睡眠模式中唤醒
