...
{
   /* 外设时钟使能 */
    ADCx_RCC_CLK_ENABLE();     // __HAL_RCC_ADC1_CLK_ENABLE()
    
    /* DMA时钟使能 */
    DMAx_RCC_CLK_ENABLE();     // __HAL_RCC_DMA1_CLK_ENABLE()
...
void ADCx_Init(void)
{
  ...
  // 配置采样通道
  sConfig.Channel = ADC_CHANNEL;    // ADC_CHANNEL_TEMPSENSOR 
  sConfig.Rank = 1;
