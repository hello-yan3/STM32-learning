...
{   
  ...
  /* 启动AD转换并使能DMA传输和中断 */
	/* 注意必须先启动DMA2，在多重启动DMA1 */
  HAL_ADC_Start(&hadcx2);
	// 参数分别为：句柄结构体，数据存放地址，数据长度(多通道的通道数)
  HAL_ADCEx_MultiModeStart_DMA(&hadcx1, &ADC_ConvertedValue, 1);
  ...
  while (1)
  {
    ...
    /* 3.3为AD转换的参考电压值，stm32的AD转换为12bit，2^12=4096，
       即当输入为3.3V时，AD转换结果为4096 */
		// 双ADC采集，ADC2采集到数据保存在ADC1的高16位，因此DMA配置数据32位，
		// 采集结果搬运到1个32位变量中，数据会依次保存在低16位，高16位
