...
static void Rheostat_ADC_Mode_Config(void)
{
    ...
    // 将ADC与DMA联系起来
    // ADC_Handle，是ADC的句柄结构体
    // hdma_adcx，是DMA的句柄结构体
    // DMA_Handle 是外设/DMA联系的桥梁
    // DMA_Handle 是DMA_HandleTypeDef类型结构体指针变量 (DMA_HandleTypeDef  *DMA_Handle;)
    // PS：DMA_HandleTypeDef类型 在 ADC_HandleTypeDef中定义，
    // 通过这个结构体指针 DMA_HandleTypeDef * DMA_Handle 完成外设/存储器 的DMA连接
