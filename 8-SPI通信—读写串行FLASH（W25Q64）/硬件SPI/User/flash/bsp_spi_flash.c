...
void SPI_FLASH_Init(void)
{
  ...
  SpiHandle.Init.DataSize          = SPI_DATASIZE_8BIT;          // 设置SPI的数据大小，通讯数据帧长度8bit
	SpiHandle.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_4;    // 设置SPI外设的时钟分频因子，使用4分频，时钟频率 36M/4 *2 = 18MHz
  ...
  SpiHandle.Init.CLKPhase          = SPI_PHASE_2EDGE;            // 设置SPI的时钟相位CPHA，选择偶数边沿采样，对应模式3，实测使用模式1也可
  SpiHandle.Init.CLKPolarity       = SPI_POLARITY_HIGH;          // 设置SPI的时钟极性CPOL，时钟空闲时刻高电平
...
//  WRITE_REG(SpiHandle.Instance->DR, TxData);
...
//  }
	...
	/* 直接使用库函数写入数据 */
	// SPI句柄结构体，发送数据缓冲区地址，发送数据大小，函数持续时间
	if( HAL_SPI_Transmit(&SpiHandle, pBuffer, NumByteToWrite, 0xFFF)!=HAL_OK )  
		SPI_TIMEOUT_UserCallback(1);
...
//  }
	/* 直接使用库函数读取数据 */
	// SPI句柄结构体，接收数据缓冲区地址，发送数据大小，函数持续时间
	if( HAL_SPI_Receive(&SpiHandle, pBuffer, NumByteToRead, 0xFFF)!=HAL_OK )  
		SPI_TIMEOUT_UserCallback(1);
