...
int main(void)
{
  ...
  ILI9341_DispString_EN ( 20, 20, "This is a MPU6050 demo" );
     ...
     * mpu_set_bypass(1);
		如果您不使用 MPU9150 并且不使用 DMP 功能，则此功能会将所有从属设备置于主总线上。 mpu_set_bypass(1);
     */
		/* 这里inv都没有用到的…… */
				/* 这里inv都没有用到的…… */
				/* 这里inv都没有用到的…… */
		// 所以这部分到时候！注意一下！要不要直接搬过去还是什么？
		// 最后怎么写程序？
...
#endif
    /* Set chip-to-body orientation matrix.		设置芯片到主体的方向矩阵。
     * Set hardware units to dps/g's/degrees scaling factor.  将硬件单位设置为 dps/g/度缩放因子。
     */
...
/* 没啥用，也不要！ */
 ...
 //加载dmp固件
    ...
    dmp_set_orientation(//设置陀螺仪方向
        inv_orientation_matrix_to_scalar(gyro_pdata.orientation));
    dmp_register_tap_cb(tap_cb);//注册要在点击事件上执行的函数。 * 点击方向由以下之一表示：
...
//！！！这个东西里面调用了自检！！！
    ...
    if (hal.motion_int_mode) {
        ...
        while (!hal.new_gyro) {}
					// 如果是0，循环，直到变为1  继续向下！天才！
        /* Restore the previous sensor configuration. */
        mpu_lp_motion_interrupt(0, 0, 0);
        hal.motion_int_mode = 0;
    }
// hal.sensors是串口收到数据处理！默认0 → 1  总之就是1对了！
    if (!hal.sensors || !hal.new_gyro) {
        ...
        if (hal.new_gyro && hal.lp_accel_mode) {
             ...
             */
					...
					/*
					当DMP正在使用时，此函数从FIFO获取新数据。
					FIFO可以包含陀螺仪、加速度、四元数和手势数据的任何组合。
					传感器参数告诉调用者哪些数据字段实际上填充了新数据。
					例如，如果传感器==（INV_XYZ_GYRO|INV_WXYZ_QUAT），
					则FIFO未填充加速度数据。驾驶员解析手势数据以确定手势事件是否已经发生；
					在事件中，应用程序将通过回调得到通知（假设回调函数已正确注册）。
...
//else if (hal.new_gyro && hal.dmp_on) {
//            short gyro[3], accel_short[3], sensors;！！！这里也有！
//            unsigned char more;
//            long accel[3], quat[4], temperature;！！！！在这里！
//						
						
						// sensor！！是标志位！标志哪个参数为1的！！！
