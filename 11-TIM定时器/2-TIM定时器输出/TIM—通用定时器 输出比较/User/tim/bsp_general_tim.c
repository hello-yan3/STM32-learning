...
__IO uint16_t OC_Pulse[2][4] = {{2000, 8000, 7000 ,3000},
			 							      		  {2500, 2500, 2500 ,2500}};
 ...
 *-----------------------------------------------------------------------------
 * 定时时间计算：
 * 时钟因子：1/(72MHz/(TIMx_PRESCALER+1)) = 1/(720M/72) = 0.1ms
 * TIM_CHANNEL_3:
...
static void TIM_Mode_Config(void)
{
  ...
  // 自动重装载寄存器的值，累计TIM_Period+1个频率后产生一个更新或者中断
	TIM_TimeBaseStructure.Init.Period = GENERAL_TIM_PERIOD;         // (0xFFFF) 
  // 时钟预分频数
  ...
  TIM_OCMODE_INACTIVE 无效电平模式，CNT=CCR时，REF置无效电平，几乎不用；
	...
	*/
	...
	TIM_OCInitStructure.Pulse = OC_Pulse[0][0];          
	// PWM通道1输出引脚有效电平极性（低电平）
	TIM_OCInitStructure.OCPolarity = TIM_OCPOLARITY_LOW;       

	//初始化通道3输出PWM 
	HAL_TIM_OC_ConfigChannel(&TIM_TimeBaseStructure,&TIM_OCInitStructure,GENERAL_TIM_CHANNELx1);   // GENERAL_TIM_CHANNELx1  TIM_CHANNEL_3
	
	// PWM CCR值(捕获/比较值) 设置 (CH4)
	TIM_OCInitStructure.Pulse = OC_Pulse[1][0];   
	//初始化通道4输出PWM 
	HAL_TIM_OC_ConfigChannel(&TIM_TimeBaseStructure,&TIM_OCInitStructure,GENERAL_TIM_CHANNELx2);   // GENERAL_TIM_CHANNELx2  TIM_CHANNEL_4
...
void HAL_TIM_OC_DelayElapsedCallback(TIM_HandleTypeDef *htim)
{
  ...
  {
    __HAL_TIM_SET_COMPARE(htim, GENERAL_TIM_CHANNELx1, count + OC_Pulse[0][i]);
		i++;
		if(i>3)  i=0; 
  }
  if(htim->Channel == GENERAL_TIM_CHANNELx2-4)
  {
    __HAL_TIM_SET_COMPARE(htim, GENERAL_TIM_CHANNELx2, count + OC_Pulse[1][j]);
		j++;
		if(j>3)  j=0; 
  }
}
