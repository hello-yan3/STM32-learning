...
{ 
  ...
  while (1)
  {
		...
		printf("当前OC_Pulse值为: %d\n",OC_Pulse);
		printf("编码器当前旋转方向: %d, 1表示计数器向下计数, 0表示向上计数\n", direction);
		printf("单位时间%dms内编码器次数为：%d \n",TIME, Encoder_Count);
		printf("当前编码器旋转速度为：%f 方波/ms \n", ABS(velocity));
		printf("当前PWM波周期为：%f ms \n",ABS(RATIO/velocity));
		printf("实际通过定时器计算得到的方波周期为: %f ms \n\n",(float)1/(72000000/(GENERAL_TIM_PRESCALER+1))*OC_Pulse*4*1000);
