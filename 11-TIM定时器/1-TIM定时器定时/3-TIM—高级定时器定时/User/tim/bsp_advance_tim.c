 ...
 void TIM_Mode_Config(void)
{
  ...
  TIM_TimeBaseStructure.Instance = ADVANCED_TIMx;      // TIM1
  ...
  // 重复计数器的值，只有高级定时器才有
  TIM_TimeBaseStructure.Init.RepetitionCounter = ADVANCED_TIM_REPETITIONCOUNTER;  // 重复计数(10-1)次
  // 初始化定时器
...
{  
    /* 基本定时器外设时钟使能 */
    ADVANCED_TIM_RCC_CLK_ENABLE();   // __HAL_RCC_TIM1_CLK_ENABLE()

    /* 外设中断配置 */
    HAL_NVIC_SetPriority(ADVANCE_TIM_IRQn , 1, 0);   // TIM1_UP_IRQn
    HAL_NVIC_EnableIRQ(ADVANCE_TIM_IRQn );           // TIM1_UP_IRQn
}
