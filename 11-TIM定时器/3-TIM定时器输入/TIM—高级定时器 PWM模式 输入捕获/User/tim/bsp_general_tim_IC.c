...
static void TIMx_IC_GPIO_Config(void) 
{
	...
	/*开启定时器相关的GPIO外设时钟*/
	GENERAL_TIM_IC_GPIO_CLK_ENABLE();   // __HAL_RCC_GPIOC_CLK_ENABLE()

	/* 定时器功能引脚初始化 */											
	GPIO_InitStructure.Pin = GENERAL_TIM_IC_PIN;	  // GPIO_PIN_6
	GPIO_InitStructure.Mode = GPIO_MODE_AF_INPUT;    
	GPIO_InitStructure.Pull = GPIO_NOPULL;
	HAL_GPIO_Init(GENERAL_TIM_IC_GPIO_PORT, &GPIO_InitStructure);	  // GPIOC
...
{
	// 开启TIM3_CLK
	GENERAL_TIM_IC_CLK_ENABLE();    // __HAL_RCC_TIM8_CLK_ENABLE()
	
  /*--------------------时基结构体初始化-------------------------*/
	TIM_TimeBaseStructure_IC.Instance = GENERAL_TIM_IC;      // TIM8
