...
static void TIMx_OC_GPIO_Config(void) 
{
	...
	/*开启定时器相关的GPIO外设时钟*/
	GENERAL_CH_OC_GPIO_CLK_ENABLE();   // __HAL_RCC_GPIOB_CLK_ENABLE()
	
	/* 定时器功能引脚初始化 */															   
	GPIO_InitStructure.Pin = GENERAL_CH_OC_PIN;	  // GPIO_PIN_10
	GPIO_InitStructure.Mode = GPIO_MODE_AF_PP;    
	GPIO_InitStructure.Pull = GPIO_NOPULL;
	GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_HIGH; 	
	HAL_GPIO_Init(GENERAL_CH_OC_GPIO_PORT, &GPIO_InitStructure);	  // GPIOB

	/* GPIO重映射 */
	__HAL_RCC_AFIO_CLK_ENABLE();         // 开启AFIO的时钟
	__HAL_AFIO_REMAP_SWJ_NOJTAG();       // 这里注意重映射到PB10必须禁用JTAG
	GENERAL_CH_OC_AFIO();                // __HAL_AFIO_REMAP_TIM2_PARTIAL_2()
	
	/* 注意：复用需要参考设置：
		 PARTIAL_2: Partial remap (CH1/ETR/PA0,  CH2/PA1, CH3/PB10, CH4/PB11)
		 这里使用PB10,所以使用这条复用语句
	
	// __HAL_AFIO_REMAP_TIM2_ENABLE();       ENABLE: Full remap       (CH1/ETR/PA15, CH2/PB3, CH3/PB10, CH4/PB11)   // 通道1、2、3、4全部复用
	// __HAL_AFIO_REMAP_TIM2_PARTIAL_2();    PARTIAL_2: Partial remap (CH1/ETR/PA0,  CH2/PA1, CH3/PB10, CH4/PB11)   // 只复用通道3、4
	// __HAL_AFIO_REMAP_TIM2_PARTIAL_1();    PARTIAL_1: Partial remap (CH1/ETR/PA15, CH2/PB3, CH3/PA2,  CH4/PA3)    // 只复用通道1、2
...
{
	// 开启TIM2_CLK
	GENERAL_TIM_OC_CLK_ENABLE();    // __HAL_RCC_TIM2_CLK_ENABLE()
	
  /*--------------------时基结构体初始化-------------------------*/
	TIM_TimeBaseStructure_OC.Instance = GENERAL_TIM_OC;      // TIM2
  // 自动重装载寄存器的值，累计TIM_Period+1个频率后产生一个更新或者中断
	TIM_TimeBaseStructure_OC.Init.Period = GENERAL_TIM_OC_PERIOD;          // (1000-1) 
  // 时钟预分频数
  ...
  HAL_TIM_ConfigClockSource(&TIM_TimeBaseStructure_OC, &sClockSourceConfig);
	...
	// PWM CCR值(捕获/比较值) 设置 
	TIM_OCInitStructure.Pulse = GENERAL_TIM_OC_CH_CCR;        // 500
